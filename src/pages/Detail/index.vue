        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  @click="changId(spuSaleAttrValue, item.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="getShopList">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods:{
    //产品的售卖属性切换高亮
    changId(spuSaleAttrValue,arr){
      //遍历所有属性值，让其高亮属性isChecked 为0，，拍他思想，干掉其它，为我独尊
       arr.forEach(element => {
         element.isChecked = 0
       });
       spuSaleAttrValue.isChecked =1
    },
    //购物车用户输入值判断，输入框一失去焦点就触发
    changeNum(event){
    //用户输入进来的文本
   ...
   async getShopList(){
    ...
    //这里我们只让query带一个简单的skuNum就好
    //方法一：$bus兄弟组件传参数   方法二：本地存储（持久localStorage）|会话存储(临时，sessionStorage)  这里我们用本地存储
    sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
    //会话/本地存储 只能存储字符串，用的时候再转化为需要用的对象或数组，这里用JSON.stringfy转化先转化为字符串
